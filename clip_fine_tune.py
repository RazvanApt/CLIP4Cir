...
from data_utils import base_path, squarepad_transform, targetpad_transform, CIRRDataset, FashionIQDataset, CSSDataset, num_workers
from utils import collate_fn, update_train_running_results, set_train_bar_description, extract_index_features, \
    save_model, generate_randomized_fiq_caption, element_wise_sum, generate_randomized_css_caption, device
from validate import compute_cirr_val_metrics, compute_fiq_val_metrics, compute_css_val_metrics

def clip_finetune_css(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
                      validation_frequency: int, transform: str, save_training: bool, encoder: str, save_best: bool,
                      **kwargs):
    """
    Fine-tune CLIP on the CSS dataset using as combining function the image-text element-wise sum
    ...
    training_path: Path = Path(
        base_path / f"models/clip_finetuned_on_css_{clip_model_name}_{training_start}")
    ...
    relative_val_dataset = CSSDataset('val', 'relative', preprocess, )
    classic_val_dataset = CSSDataset('val', 'classic', preprocess, )
    ...
    relative_train_dataset = CSSDataset('train', 'relative', preprocess)
    ...
    for epoch in range(num_epochs):
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):
                ...
                captions = generate_randomized_css_caption(flattened_captions)
        ...
        if epoch % validation_frequency == 0:
            with experiment.validate():
                ...
                # Compute and log validation metrics for validation dataset of CSS
                if encoder == 'text':
                    index_features, index_names = index_features_list, index_names_list
                else:
                    index_features, index_names = extract_index_features(classic_val_dataset, clip_model)
                recall_at10, recall_at50 = compute_css_val_metrics(relative_val_dataset, clip_model,
                                                                    index_features, index_names,
                                                                    combining_function)

                results_dict = {}
                
                results_dict['recall_at10'] = recalls_at10
                results_dict['recall_at50'] = recalls_at50
                
                results_dict.update({
                    f'recall_at10': recalls_at10,
                    f'recall_at50': recalls_at50,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, required=True, help="should be either 'CIRR', 'fashionIQ' or 'CSS")
    ...
    if args.dataset.lower() not in ['fashioniq', 'cirr', 'css']:
