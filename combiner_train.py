...
from data_utils import base_path, squarepad_transform, FashionIQDataset, targetpad_transform, CIRRDataset, CSSDataset, num_workers
from combiner import Combiner
from utils import collate_fn, update_train_running_results, set_train_bar_description, save_model, \
    extract_index_features, generate_randomized_fiq_caption, generate_randomized_css_caption, device
...
def combiner_training_css(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
                          combiner_lr: float, batch_size: int, clip_bs: int, validation_frequency: int,
                          transform: str, save_training: bool, save_best: bool, **kwargs):
    """
    Train the Combiner on CSS dataset keeping frozed the CLIP model
    :param projection_dim: Combiner projection dimension
    :param hidden_dim: Combiner hidden dimension
    :param num_epochs: number of epochs
    :param clip_model_name: CLIP model you want to use: "RN50", "RN101", "RN50x4"...
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    training_path: Path = Path(
        base_path / f"models\combiner_trained_on_css_{clip_model_name}_{training_start}")
    # print("Trainign path: ", training_path)
    training_path.mkdir(exist_ok=False, parents=True)
    ...
    index_features_and_names = extract_index_features(classic_val_dataset, clip_model)
    index_features = index_features_and_names[0]
    index_names = index_features_and_names[1]
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                # Randomize the training caption in four way: (a) cap1 and cap2 (b) cap2 and cap1 (c) cap1 (d) cap2
                flattened_captions: list = np.array(captions).T.flatten().tolist()
                input_captions = generate_randomized_css_caption(flattened_captions)
                text_inputs = clip.tokenize(input_captions, truncate=True).to(device, non_blocking=True)
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                ...
                # Compute and log validation metrics for validation dataset
                recall_at10, recall_at50 = compute_css_val_metrics(relative_val_dataset, clip_model, index_features,
                                                                    index_names, combiner.combine_features)
...
if __name__ == '__main__':
    ...
    #   combiner_training_css(**training_hyper_params)
      ...
      print("combiner_train.py: Dataset is not CSS, FashionIQ, CIRR")
